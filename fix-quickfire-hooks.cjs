...
// Find and remove the existing QuickFire component
const startMarker = '\n// =============================================================\n// QUICKFIRE';
const startIdx = lsContent.indexOf(startMarker);

if (startIdx === -1) {
  console.log('ERROR: Could not find QuickFire component');
  process.exit(1);
}

console.log(`Found QuickFire at index ${startIdx}`);
lsContent = lsContent.substring(0, startIdx);

// Add corrected QuickFire using named hooks
...
console.log('SUCCESS: QuickFire component replaced with fixed version');
